				...
				if len(times[thisTime]) > 1:
					...
					for cell in cells:
						if cell != minCell:
							...
							cell['track'] = 'NaN'
							modifiedCells[list(stormCells.keys())[list(stormCells.values()).index(cell)]] = cell #Added list() - JS
		...
		for date in np.unique(dates):
			...
			for i in range(0, mainIters):
				...
				for k in xrange(0, len(stormTracks.keys()), numPerProc):
					temp = {}
					for key in list(stormTracks.keys())[k:k + numPerProc]:  #Added list() - JS
                ...
                #Commented out the following two as they appear to have already been assigned values before this declaration -JS
				# global stormTracksG
				# global stormCellsG
				stormTracksG = stormTracks
		...
		for track in stormTracks:
			...
			if bigData:
				if stormTracks[track]['t0'].date() != date and stormTracks[track]['tend'].date() != date:
                    ...
                    #Added list() - JS
					if list(stormTracks.keys()).index(track) % REPORT_EVERY == 0: print('......' + str(list(stormTracks.keys()).index(track)) + ' of ' + str(len(stormTracks)) + ' tracks processed......')
					continue
			...
			for cell in stormTracks[track]['cells']:
				...
				ids.append(list(activeStormCells.keys())[list(activeStormCells.values()).index(cell)]) #Added list() - JS
			...
			if list(stormTracks.keys()).index(track) % REPORT_EVERY == 0: print('......' + str(list(stormTracks.keys()).index(track)) + ' of ' + str(totNumTracks) + ' tracks processed......') #Added list() - JS
