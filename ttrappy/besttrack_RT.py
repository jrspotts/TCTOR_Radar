...
import json
#import geojson Commented out since it is not found and this appears the only place it's used in this file
import sys
import os
import numpy as np
import datetime
...
def readSegmotionXML(inDir, inSuffix, historyPath, startTime, endTime):
    ...
    try:
        ...
        #Added the following to convert loaded dictionary keys to int. json.dump does not like strings as keys when sorting - JS
        stormCells = {int(key):value for key, value in stormCells.items()}
...
def outputXML(currentTime, newCells, stormCells, changedCells, outDir, historyPath, xmlFile):
    ...
    for line in lines:
        if not line.strip().startswith('<data>'):
            ...
            break
    f.write(' <data>\n')
    names = [column[0] for column in newCells[list(newCells.keys())[0]]['xml']]
    units = [column[1] for column in newCells[list(newCells.keys())[0]]['xml']]
    ...
    for cell in newCells:
        ...
        for i in range(0, len(newCells[cell]['xml'])):
            if names[i] == 'RowName': 
                names[i] = 'RowName' #Renamed to RowName from Track to be compatible with w2hailtruth_size
		...
		for track in stormTracks:
			...
			for i in range(len(ids)):
				if ids[i] in newCells:
					...
					if type(stormCells[ids[0]]['start']) is str: # Yes I'm still fighting this stupid issue       (Changed unicode to str - JS)
